...
//   - Read a token from the file and verify that it is "Mesh".
//     - Read a token and store it in the Mesh's name variable.
//     - Read an integer indicating the number of vertices to be read.
//     - Tell the DGL to start a mesh.
//     - For each vertex,
//       - Read a Vector2D (position), a DGL_Color (color), and a Vector2D (UV).
//       - Call DGL_Graphics_AddVertex() to add a single vertex to the mesh.
//     - Tell the DGL to end a mesh and store the returned resource.
//     - Set the appropriate draw mode.)
// Params:
